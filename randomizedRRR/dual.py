...
import numpy as np
import torch
from scipy.linalg import LinAlgError, eig, eigh, pinvh
from scipy.sparse.linalg import eigs
from torch import cholesky_solve
from torch.linalg import cholesky
from utils import frnp, tonp
...
def fit_rand_reduced_rank_regression_tikhonov(
    ...
    if optimal_sketching:
        Cov = L
        Om = torch.tensor(
            rng.multivariate_normal(np.zeros(dim), tonp(Cov, cuda), size=l).T,
            dtype=K_X.dtype,
            device=device,
        )
    else:
        Om = torch.tensor(
            rng.standard_normal(size=(dim, l)), dtype=K_X.dtype, device=device
        )
...
def fit_reduced_rank_regression_tikhonov(
    ...
    if svd_solver == "arnoldi":
        ...
        sigma_sq, U = eigs(
            tonp(K, cuda), rank + 3, tonp(regularize(K_X, tikhonov_reg), cuda)
        )
    else:  # 'full'
    ...
    U, _, columns_permutation = modified_QR(
        U, M=tonp(norm_inducing_op, cuda), column_pivoting=True
    )
